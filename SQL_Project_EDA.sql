...
ADD COLUMN corrected_date DATE AFTER location;	-- add a new column with the corrected date format next to the column 'location'
...
# Looking at 'total_cases' against 'total_deaths'
SELECT location, date, total_cases, total_deaths, ROUND((total_deaths / total_cases) * 100, 2) AS death_percentage	-- with a result column for percentage likelihood of dying if contracted, in your country 
FROM covid_deaths
...
SELECT location, date, population, total_cases, (total_cases / population) * 100 AS population_percentage	-- with a result column for population percentage who got contracted
FROM covid_deaths
...
# Observing it down by continent
...
-- if you want just the global total, take out column 'date' and GROUP BY date
...
-- we are going to add a rolling count on the 'new_vaccinations' using Window Functions
SELECT d.continent, d.location, d.date, d.population, v.new_vaccinations, 
...
-- using CTE
WITH pop_vacs (continent, location, date, population, new_vaccinations, rolling_vaccinations) AS 
	(	SELECT d.continent, d.location, d.date, d.population, v.new_vaccinations, 
		SUM(v.new_vaccinations)	OVER (PARTITION BY d. location ORDER BY d.location, d.date) AS rolling_vaccinations
		FROM covid_deaths d
		JOIN covid_vaccination v
